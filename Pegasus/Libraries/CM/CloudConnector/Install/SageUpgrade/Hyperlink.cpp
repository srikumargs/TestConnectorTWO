...
CHyperlink::CHyperlink(std::ofstream_t& oLogStream, const CTextAttributes& oTextAttributes, CommandType eCommandType, const std::string_t& sCommand, const std::string_t& sCommandParameters, CBrowserWindow* pBrowserWindow)
: m_hwnd(NULL), 
m_bMouseInWindow(false),
m_oTextAttributes(oTextAttributes),
...
m_oLogStream(oLogStream),
m_pBrowserWindow(pBrowserWindow)
{
}
...
int CHyperlink::WndProc(HWND hwnd, WORD wMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		...
		case ctExit:
			if(pHyperlink->m_pBrowserWindow->IsUpgradeInProgress())
			{
				::MessageBox(pHyperlink->m_pBrowserWindow->GetHWND(), _T("Please wait for the update process to complete before closing."), _T("Update In Progress"), MB_OK|MB_ICONERROR);
			}
			else
			{
		...
		case ctUpgradeProcess:
			if(pHyperlink->m_pBrowserWindow->IsUpgradeInProgress())
			{
				::MessageBox(pHyperlink->m_pBrowserWindow->GetHWND(), _T("Please wait for the update process to complete before starting again."), _T("Update In Progress"), MB_OK|MB_ICONERROR);
			}
			else
			{
				std::string_t sMessage;
				sMessage += _T("Before updating you must close ");
				sMessage += GetBriefProductName();
				sMessage += _T(".\n\nAre you ready to update now?");
				if(IDYES==::MessageBox(pHyperlink->m_pBrowserWindow->GetHWND(), sMessage.c_str(), _T("Update Confirmation"), MB_YESNO|MB_ICONQUESTION))
				{
					pHyperlink->m_pBrowserWindow->AdvanceUpgradeState(WM_UPGRADE_STARTING, _T("Starting update..."));
				}
			}
			break;
		}

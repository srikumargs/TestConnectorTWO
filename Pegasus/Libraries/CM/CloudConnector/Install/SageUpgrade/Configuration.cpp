...
m_bAutoUpgrade(false)
{
	...
	GetTempFileName(tszPath, _T("SU-"), 0, tszTempName);
	...
	m_optionHandlers.insert(std::make_pair(std::string_t(_T("autoUpgrade")), &CConfiguration::AutoUpgradeOptionHandler));
	...
	m_sImageFilePath = ExpandToFullPath(oIniFile.ReadString(_T("General"), _T("ImageFilePath"), _T("SageUpgrade.bmp")));
...
void CConfiguration::AutoUpgradeOptionHandler(const std::string_t&)
{
	m_bAutoUpgrade = true;
}

std::string_t CConfiguration::ExpandToFullPath(const std::string_t& sPath) const
{
	if(sPath.empty())
	{

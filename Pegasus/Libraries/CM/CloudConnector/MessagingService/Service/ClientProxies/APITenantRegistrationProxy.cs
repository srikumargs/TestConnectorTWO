...
{
    internal sealed class APITenantRegistrationProxy : APIBaseServiceProxy
    {
        public APITenantRegistrationProxy(
            Uri endpointAddress,
            string resoucePath,
            string tenantId,
            PremiseAgent premiseAgent,
            MessageLogger logger) : base(endpointAddress, resoucePath, tenantId, string.Empty, string.Empty, premiseAgent, logger)
        {
        }

        public Cloud.Integration.Interfaces.WebAPI.TenantRegistration RegisterTenant(string companyId, string authenticationToken)
        {

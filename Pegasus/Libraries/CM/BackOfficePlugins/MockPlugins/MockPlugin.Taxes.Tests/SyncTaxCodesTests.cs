...
namespace MockPlugin.Taxes.Tests
{
    [TestClass]
    public class SyncTaxCodesTests : CatalogInitialization
    {
        [ImportMany]
...
#pragma warning restore 649
        ...
        /// </summary>
        [TestMethod]
        public void TestSyncTaxCodes()
        {
            ISyncTaxCodes backofficeFeatureProcessor = null;
            ...
            try
            {
                ...
                Assert.IsNotNull(backofficeFeatureProcessor, String.Format("{0} BackOffice to Sync TaxCodes was not found!",_backOfficeCompanyData.BackOfficeId));

                var backOfficeSessionHandler = backofficeFeatureProcessor as IBackOfficeSessionHandler;

                Assert.IsNotNull(backOfficeSessionHandler, "Missing BackOffice Session handler");

                //Begin back office session with configuration in order to make back office connections.
                var clientResponse = backOfficeSessionHandler.BeginSession(_sessionContext, _backOfficeCompanyData);
                ...
                clientResponse = backofficeFeatureProcessor.InitializeSyncTaxCodes();

                Assert.IsNotNull(clientResponse, String.Format("InitializeSyncTaxCodes Failed for Back Office {0}.",_backOfficeCompanyData.BackOfficeId));
                Assert.AreNotEqual(Status.Failure, clientResponse.Status);

                //Loop  through sync logic to get the set of TaxCode payload. 
                var taxCode = backofficeFeatureProcessor.GetNextSyncTaxCode();
                var count = 0;
                while (taxCode != null)
                {
                    count++;
                    //todo:  each back office can decide how to test on return information 
                    Assert.AreEqual((10 + count).ToString(CultureInfo.CurrentCulture), taxCode.ExternalId, "unexpected TaxCode data");

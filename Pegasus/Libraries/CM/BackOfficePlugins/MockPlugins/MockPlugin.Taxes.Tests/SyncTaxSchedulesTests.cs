...
{
    [TestClass]
    public class SyncTaxSchedulesTests : CatalogInitialization
    {
        [ImportMany]
...
#pragma warning restore 649
        ...
        /// <summary>
        /// Sync Tax Schedules test  
        /// This is a happy path test, meaning null data or response status of failure will cause the test to fail.
        /// </summary>
        [TestMethod]
        public void TestSyncTaxSchedules()
        {
            ISyncTaxSchedules backofficeFeatureProcessor = null;
            ...
            try
            {
                ...
                Assert.IsNotNull(backofficeFeatureProcessor, String.Format("{0} BackOffice to Sync TaxSchedules was not found!",_backOfficeCompanyData.BackOfficeId));
                ...
                clientResponse = backofficeFeatureProcessor.InitializeSyncTaxSchedules();

                Assert.IsNotNull(clientResponse, String.Format("InitializeSyncTaxSchedules Failed for Back Office {0}.",_backOfficeCompanyData.BackOfficeId));
                Assert.AreNotEqual(Status.Failure, clientResponse.Status);

                //Loop  through sync logic to get the set of TaxSchedule payload. 
                var taxSchedule = backofficeFeatureProcessor.GetNextSyncTaxSchedule();
                var count = 0;
                while (taxSchedule != null)
                {
                    count++;
                    //todo:  each back office can decide how to test on return information 
                    Assert.AreEqual((1000 + count).ToString(CultureInfo.CurrentCulture), taxSchedule.ExternalId, "unexpected TaxSchedule data");

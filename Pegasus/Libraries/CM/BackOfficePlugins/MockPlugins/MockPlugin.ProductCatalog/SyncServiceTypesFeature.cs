...
{
    /// <summary>
    /// Mock backoffice plugin for Sync ServiceTypes. 
    /// </summary>
    [Export(typeof(ISyncServiceTypes))]
    [ExportMetadata("BackOfficeId", "Mock")]
    public class SyncServiceTypesFeature : ISyncServiceTypes
    {
        private int _count;
        ...
        /// </returns>
        public Response InitializeSyncServiceTypes()
        {
            _count = 0;
        ...
        /// <summary>
        /// Get the next ServiceType that needs to be sync'd up to the cloud
        /// </summary>
        /// <returns></returns>
        public ServiceType GetNextSyncServiceType()
        {
            ...
            if (multiple)
            {
                for (var i = 0; i < 3; i++)
                {
                    serviceType.TaxClasses.Add(new ServiceTypeTaxClass
                    {
                        ExternalId = extId + "-" + i,
                        ExternalReference = extId + "-" + i,
                        EntityStatus = EntityStatus.Active,
                        TaxClass = "Serv" + i,
                        TaxCode = (10 + i).ToString(CultureInfo.CurrentCulture)
                    });
                }
            }
            else
            {
                serviceType.TaxClass = "Labor";
            }

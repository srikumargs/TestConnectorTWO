...
namespace MockPlugin.Customers.Tests
{
    [TestClass]
    public class SyncCustomersTests : CatalogInitialization
    {
        [ImportMany]
...
#pragma warning restore 649
        ...
        /// </summary>
        [TestMethod]
        public void TestSyncCustomers()
        {
            ISyncCustomers backofficeFeatureProcessor = null;
            ...
            try
            {
                ...
                Assert.IsNotNull(backofficeFeatureProcessor, String.Format("{0} BackOffice to Sync Customers was not found!",_backOfficeCompanyData.BackOfficeId));
                ...
                if (initDefaultProperties != null)
                {
                    ...
                    defaultPropertyValues.Add("PaymentTerms", "Terms30Days");
                    defaultPropertyValues.Add("ListField", "List Item 3");
                ...
                clientResponse = backofficeFeatureProcessor.InitializeSyncCustomers();

                Assert.IsNotNull(clientResponse, String.Format("InitializeSyncCustomers Failed for Back Office {0}.",_backOfficeCompanyData.BackOfficeId));
                Assert.AreNotEqual(Status.Failure, clientResponse.Status);
                //Loop  through sync logic to get the set of customer payload. 
                var customer = backofficeFeatureProcessor.GetNextSyncCustomer();
                var count = 0;
                while (customer != null)
                {
                    count++;
                    //todo:  each back office can decide how to test on return information 
                    Assert.AreEqual((1000 + count).ToString(CultureInfo.CurrentCulture), customer.ExternalId, "unexpected customer data");

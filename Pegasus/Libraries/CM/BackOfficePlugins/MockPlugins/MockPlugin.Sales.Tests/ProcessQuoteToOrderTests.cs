...
{
    [TestClass]
    public class ProcessQuoteToOrderToOrderTests : CatalogInitialization
    {
        [ImportMany]
#pragma warning disable 649
        private IEnumerable<Lazy<IProcessQuoteToOrder, IBackOfficeData>> _backOfficeHandlers;
#pragma warning restore 649
        ...
        /// <summary>
        /// Process Quote To Order test  
        /// This is a happy path test, meaning null data or response status of failure will cause the test to fail.
        /// </summary>
        [TestMethod]
        public void TestProcessQuoteToOrder()
        {
            IProcessQuoteToOrder backofficeFeatureProcessor = null;
            try
            {
                //===========================================================================================================================================================
                // Process Code
                ...
                Assert.IsNotNull(backofficeFeatureProcessor, String.Format("{0} BackOffice to Process Quote To Order was not found!",_backOfficeCompanyData.BackOfficeId));
                ...
                var quoteToOrder = CreateQuoteToOrder();

                var response = backofficeFeatureProcessor.ProcessQuoteToOrder(quoteToOrder);
                Assert.IsNotNull(response, "Null response from ProcessQuoteToOrder");
                Assert.AreNotEqual(Status.Failure, response.Status, "Failure status returned from ProcessQuoteToOrder.");

                Assert.IsNotNull(quoteToOrder, "quoteToOrder came back as null");

                //Expecting same count in details as delivered. 
                Assert.AreEqual(5, quoteToOrder.Details.Count);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// TODO by backoffice plugin developer:  Need to set up data that the erp can process based on quoteToOrder contract. 
        /// </summary>
        /// <returns></returns>
        private QuoteToOrder CreateQuoteToOrder()
        {
            var quoteToOrder = new QuoteToOrder
            {
                ...
                {
                    //TODO: ask back office if they need an external id for address in this sales domain process quoteToOrder contract
                    ...
                    {
                        //TODO: ask back office if they need an external id for contact in this sales domain process quoteToOrder contract
                        FirstName = "Joe",
                        LastName = "Smith",
            ...
            for (var i = 0; i < 5; i++)
            {
                ...
                quoteToOrder.Details.Add(detail);

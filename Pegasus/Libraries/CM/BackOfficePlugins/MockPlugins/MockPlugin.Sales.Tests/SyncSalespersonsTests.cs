...
{
    [TestClass]
    public class SyncSalespersonsTests : CatalogInitialization
    {
        [ImportMany]
...
#pragma warning restore 649
        ...
        /// <summary>
        /// Sync Salespersons test  
        /// This is a happy path test, meaning null data or response status of failure will cause the test to fail.
        /// </summary>
        [TestMethod]
        public void TestSyncSalespersons()
        {
            ISyncSalespersons backofficeFeatureProcessor = null;
            ...
            try
            {
                ...
                Assert.IsNotNull(backofficeFeatureProcessor, String.Format("{0} BackOffice to Sync Salespersons was not found!",_backOfficeCompanyData.BackOfficeId));
                ...
                //Initialize the statements processing to allow back office to set up for the set of salespersons that will be processed.
                clientResponse = backofficeFeatureProcessor.InitializeSyncSalespersons();

                Assert.IsNotNull(clientResponse, String.Format("InitializeSyncSalespersons Failed for Back Office {0}.",_backOfficeCompanyData.BackOfficeId));
                Assert.AreNotEqual(Status.Failure, clientResponse.Status);
                //Loop  through sync logic to get the set of salesperson payload. 
                var salesperson = backofficeFeatureProcessor.GetNextSyncSalesperson();
                var count = 0;
                while (salesperson != null)
                {
                    count++;
                    //todo:  each back office can decide how to test on return information 
                    Assert.AreEqual((1000 + count).ToString(CultureInfo.CurrentCulture), salesperson.ExternalId, "unexpected salesperson data");

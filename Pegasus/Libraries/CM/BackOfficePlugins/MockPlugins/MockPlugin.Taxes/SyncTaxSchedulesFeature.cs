...
{
    /// <summary>
    /// Mock backoffice plugin for Sync TaxSchedules. 
    /// </summary>
    [Export(typeof(ISyncTaxSchedules))]
    [ExportMetadata("BackOfficeId", "Mock")]
    public class SyncTaxSchedulesFeature : ISyncTaxSchedules
    {
        ...
        public Response InitializeSyncTaxSchedules()
        {
            _count = 0;
        ...
        /// <summary>
        /// Get the next TaxSchedule that needs to be sync'd up to the cloud
        /// </summary>
        /// <returns></returns>
        public TaxSchedule GetNextSyncTaxSchedule()
        {
            ...
            TaxSchedule taxSchedule = new TaxSchedule
            {
                ...
                Description = extId + "Desc"

            };
            var detail = new TaxScheduleDetail
            {
                ExternalId = extId, // need something more unique if multiple contacts
                ExternalReference = extId,
                EntityStatus = EntityStatus.Active,
                TaxCodeSequence = 1,
                TaxCode = (10 + _count).ToString(CultureInfo.CurrentCulture)

                
            };
            taxSchedule.Details.Add(detail);

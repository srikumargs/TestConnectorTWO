...
{
    [TestClass]
    public class SyncInventoryItemsTests : CatalogInitialization
    {
        [ImportMany]
...
#pragma warning restore 649
        ...
        /// <summary>
        /// Sync Inventory Items test  
        /// This is a happy path test, meaning null data or response status of failure will cause the test to fail.
        /// </summary>
        [TestMethod]
        public void TestSyncInventoryItems()
        {
            ISyncInventoryItems backofficeFeatureProcessor = null;
            ...
            try
            {
                ...
                Assert.IsNotNull(backofficeFeatureProcessor, String.Format("{0} BackOffice to Sync InventoryItems was not found!",_backOfficeCompanyData.BackOfficeId));
                ...
                clientResponse = backofficeFeatureProcessor.InitializeSyncInventoryItems();

                Assert.IsNotNull(clientResponse, String.Format("InitializeSyncInventoryItems Failed for Back Office {0}.",_backOfficeCompanyData.BackOfficeId));
                Assert.AreNotEqual(Status.Failure, clientResponse.Status);

                //Loop  through sync logic to get the set of inventoryItem payload. 
                var inventoryItem = backofficeFeatureProcessor.GetNextSyncInventoryItem();
                var count = 0;
                while (inventoryItem != null)
                {
                    count++;
                    //todo:  each back office can decide how to test on return information 
                    Assert.AreEqual((10 + count).ToString(CultureInfo.CurrentCulture), inventoryItem.ExternalId, "unexpected inventoryItem data");
